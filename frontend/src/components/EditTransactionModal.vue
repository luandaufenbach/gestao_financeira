            ...
            <form class="grid grid-cols-1 md:grid-cols-2 gap-4" @submit.prevent="saveTransaction">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea v-model="description" rows="2"
                        class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-slate-800 focus:outline-none focus:ring-2 focus:ring-lime-300"></textarea>
                </div>
...
const {
    ...
    requiresCategory, validate, getFormData, loadTransaction
} = useTransactionForm(categories, props.transaction);

async function saveTransaction() {
    if (!validate()) return;

    try {
        const response = await update(props.transaction._id, getFormData());
